...
public class AuthGrpcService extends AuthServiceGrpc.AuthServiceImplBase {
    ...
    public void authorize(JwtRequest request, StreamObserver<JwtToken> responseObserver) {
        ...
        responseObserver.onNext(JwtToken.newBuilder().setJwtToken(Jwts.builder()
                ...
                .compact()).setUser(UserInfo.newBuilder().setUserName(user.getUserName()).setFirstName(user.getFirstName()).setLastName(user.getLastName()).setEmail(user.getEmail()).setActivo(user.isActivo()).setIdTienda(user.getIdTienda()).setId(user.getId()).setRol(user.getRol()).build()).build());
