    ...
    const handleDeleteStore = async (storeId) => {
        try {
            ...
            if (result.isConfirmed) {
                await axios.delete(`http://localhost:5000/deleteTienda/${storeId}`);
                const updatedStores = stores.filter(store => Number(store.id) !== storeId);
                        ...
                        {filteredStores.map(store => (
                            <tr key={store.codigo}>
                                ...
                                <td className="py-2 px-4 border-b">
                                    ...
                                    <button 
                                        onClick={() => handleDeleteStore(Number(store.id))}
